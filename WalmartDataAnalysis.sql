...
SELECT DISTINCT payment_method,COUNT(*) AS no_payments,
    SUM(quantity) AS quantity FROM walmart GROUP BY payment_method;
    
-- Q2 : Identify the highest-rated category in each branch
-- Display the branch, category, and avg rating
-- SELECT BRANCH,category,AVG(rating) as Rating from Walmart GROUP BY Branch,category;
...
SELECT w.branch, w.payment_method
FROM (
    SELECT branch, payment_method, COUNT(*) AS total_trans
    FROM walmart
    GROUP BY branch, payment_method
...
WHERE NOT EXISTS (
    ...
    WHERE x.branch = w.branch AND x.total_trans > w.total_trans
);
...
FROM Year2022 AS r2022
JOIN Year2023 AS r2023 ON r2022.branch = r2023.branch
